import React, { useState } from 'react'
import { ToastContainer, toast } from 'react-toastify';
import { useLocation, useNavigate } from 'react-router-dom';
import { doctorVerify } from "../services/Apis"

const Docotp = () => {
  const [otp, setOtp] = useState("");

  const location = useLocation();

  const navigate = useNavigate();

  const LoginDoctor = async (e) => {
    e.preventDefault();

    if (otp === "") {
      toast.error("Enter Your Otp")
    } else if (!/[^a-zA-Z]/.test(otp)) {
      toast.error("Enter Valid Otp")
    } else if (otp.length < 6) {
      toast.error("Otp Length Require Minimum 6 digit")
    } else {
      const data = {
        otp, email: location.state
      }
      data.email='rf@gmail.com';

      const response = await doctorVerify(data);
      console.log(response);
      return;
      if (response?.status === 200) {
        localStorage.setItem("userdbtoken", response?.data?.userToken);
        toast.success(response?.data?.message);
        setTimeout(() => {
          navigate("/Doctordetails")
        }, 5000)
      } else {
        toast.error(response?.response?.data?.error)
      }
    }
  }

  return (
    <>
      <section>
        <div className="form_data">
          <div className="form_heading">
            <h1>Doctor please Enter Your OTP Here</h1>
          </div>
          <form>
            <div className="form_input">
              <label htmlFor="otp">OTP</label>
              <input type="text" name="otp" id="" onChange={(e) => setOtp(e.target.value)} placeholder='Enter Your OTP' />
            </div>
            <button className="btn" onClick={LoginDoctor}>Submit</button>
          </form>
        </div>
        <ToastContainer />
      </section>
    </>
  )
}

export default Docotp